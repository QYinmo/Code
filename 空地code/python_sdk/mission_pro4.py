    ...
    def init_obs(self):
       ...
       #         45, 135, 3)[0].to_xy() / 10)
        ...
        for retry in range(max_retry):
            stable_points = self.radar.map.find_nearest_with_stable_ext_point(
                from_=40, to_=110, num=3, range_limit=4500, angle_tolerance=3, dist_tolerance=500)
            if len(stable_points) >= 3:
                ...
                logger.warning(
                    f"尝试 {retry+1}/{max_retry}: 只找到 {len(stable_points)} 个点")
                time.sleep(0.5)
        else:
        ...
        for i in range(3):
            ...
            logger.info(
                f"{int(point[i][0] + r*np.sin(degree[i])), int(point[i][1] - r*np.cos(degree[i]))}")
        self.obs()
