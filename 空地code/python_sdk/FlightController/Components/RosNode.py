...
class RadarListenNode(Node):
    ...
    def __init__(self, callback: Callable[[Radar_Package], None]):
        ...
        self.radar_sub = self.create_subscription(LaserScan, "/scan", self.radar_callback, 10)  # LaserScan是数据类型，/scan是topic，self.radar_callback是回调函数
...
class T265ListenNode(Node):
    '''
    监听T265数据
    T265数据也是package安装后发布
    '''
...
class MapListenNode(Node):
    '''
    监听地图数据
    地图数据由cartographer发布，在ROSMapper中
    '''
...
class Tf2ListenNode(Node):
    '''
    监听tf2数据
    这个数据不是木有用的, 是用于找到两个坐标系之间的变换关系, 相当于提供位置信息
    '''
    def __init__(
...
class RosNodeRunner:  # run all the listen nodes
    ...
    ROS 节点运行器
    (单例模式)
    一旦初始化，并且调用了add_nodes()方法，就会自动运行所有节点
    """
    ...
    def __init__(self):
        ...
        self._thread = threading.Thread(target=self._excuter.spin, daemon=True) # 很重要，因为节点的运行必须使用这个spin保持运行，守护线程
