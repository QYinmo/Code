...
class T265(object):
    ...
    def _updated(self):
        # 更新回调, 这里传递给回调函数的是T265_Pose_Frame对象, 最主要的用途为传递给飞控作通用速度输入
        if self._print_update:
            self._print_pose()
        self._update_count += 1
        # 支持多个回调(相当于可以有多个源在获取数据)
        if self._callbacks:
    ...
    def _callback_ros(self, data) -> None:
        # translate ROS topic to T265_Pose_Frame
        self.frame_num += 1
        # y -> z, x -> -y, z -> -x
    ...
    def update(self):
        ...
        frames = self._pipe.wait_for_frames()   # 阻塞直到有新的数据帧到来
