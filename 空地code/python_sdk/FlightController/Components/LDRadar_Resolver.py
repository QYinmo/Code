...
class Map_Circle(object):
    ...
    def __init__(self):
        ...
        self._deg_arr = np.linspace(0, 360, 360 * self.ACC, endpoint=False)
        self._cached_ext_points = deque(maxlen=3)
    ...
    def find_nearest_with_stable_ext_point(self, from_: float = 0, to_: float = 359, num=1, range_limit=1e7, angle_tolerance=1, dist_tolerance=500) -> List[Point_2D]:
        """
        在给定范围内查找稳定下来的极值点。
        - 每次查找后，将结果存入缓存。
        - 当缓存满3次后，将3次结果进行聚类，找出稳定的极值点。
        - angle_tolerance: 角度容差，单位为度。
        - dist_tolerance: 距离容差。
        """
        current_ext_points = self.find_nearest_with_ext_point_opt(from_, to_, num, range_limit)
        self._cached_ext_points.append(current_ext_points)

        # 如果缓存未满3次，直接返回当前结果
        if len(self._cached_ext_points) < 3:
            return current_ext_points

        all_points = sum(self._cached_ext_points, []) 
        ...
        for i in range(len(all_points)):
            ...
            for j in range(i + 1, len(all_points)):
                ...
                # 检查距离和角度是否在容差范围内
                # 将 .deg 替换为 .degree
                angle_diff = min(abs(current_point.degree - other_point.degree), 360 - abs(current_point.degree - other_point.degree))
                
                # 将 .dist 替换为 .distance
                if angle_diff < angle_tolerance and abs(current_point.distance - other_point.distance) < dist_tolerance:
                    cluster_points.append(other_point)
                    visited_indices.add(j)

            if len(cluster_points) >= 3:
                # 将 .deg 替换为 .degree
                avg_deg = sum(p.degree for p in cluster_points) / len(cluster_points)
                
                # 将 .dist 替换为 .distance
                avg_dist = sum(p.distance for p in cluster_points) / len(cluster_points)
                
                stable_points.append(Point_2D(avg_deg, avg_dist))
        
        # 将 .dist 替换为 .distance
        stable_points.sort(key=lambda p: p.distance)
        print(f"{stable_points}")    
        return stable_points[:num]
    
    def find_nearest_and_cluster(self, from_=0, to_=180, step=3,dis=3000):
            """
            在指定角度范围内，以指定步长找到每个位置的最近点，并对这些点进行聚类。
            对每个聚类计算其平均距离，如果超过距离阈值，则丢弃该聚类。

            参数:
                from_: 起始角度 (度)
                to_: 结束角度 (度)
                step: 角度步长 (度)
                distance_threshold: 聚类的平均距离阈值 (mm)

            返回:
                聚类中心列表，每个元素是一个 NumPy 数组，表示一个聚类中心的坐标。
            """
            angle = from_
            clusters = []
            while angle < to_:
                points = self.find_nearest(from_=angle, to_=angle + step, num=1)
                for point in points:
                    clusters.append(point.to_xy())
                angle += step
            if len(clusters) < 2:
                return []
            points_array = np.array(clusters)
            return self.cluster_and_filter(points_array,distance_threshold=dis)

    def cluster_and_filter(self, points, eps=400, min_samples=2, distance_threshold=2250):
            """使用DBSCAN聚类后过滤远距离簇"""
