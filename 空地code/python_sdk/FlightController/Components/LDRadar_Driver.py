...
class LD_Radar(object):
    ...
    def start(
        ...
        陆佬没讲清楚的部分1：
        留了三个模式，ROS模式，飞控转发模式，串口模式
        ROS模式不多说，用官方的package
        飞控转发模式，需要传入一个飞控对象，然后注册雷达回调函数，因为数据包在接收飞控的数据那里
        直接串口模式，经典，自己看去
        陆佬没讲清楚的事情2:
        注意那个_ros_callback，_fc_callback，这两个函数是雷达数据的回调函数，分别对应ROS和飞控转发
        飞控转发好理解，就是飞控那边有数据了要传进来，经典的面向对象的思想
        那么如果我用了ROS，为啥还要搞个回调函数？数据不是直接交给Cartographer了吗？
        因为Cartographer不是万能的，雷达扫杆这类算法仍然需要数据，所以陆佬在这里留了个接口，保证之前的算法能够直接调用。
        """
        if self.running:
            self.stop()
        self.running = True
