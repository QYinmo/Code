...
class AStar:
    ...
    def searching_repeated_astar(self, e):
        ...
        while e >= 1:  # 当权重 e 大于等于 1 时循环
            p_k, v_k = self.repeated_searching(
                self.s_start, self.s_goal, e)  # 执行一次带权重的 A* 搜索
    ...
    def repeated_searching(self, s_start, s_goal, e):
        ...
        g = {s_start: 0, s_goal: float("inf")}  # g 值（从起点到当前节点的代价）字典
        PARENT = {s_start: s_start}            # 父节点字典
        OPEN = []                              # 优先队列
        CLOSED = []                            # 已访问节点列表
        # 将起始节点加入优先队列，优先级为 g[s_start] + e * heuristic(s_start)
        heapq.heappush(OPEN,
