...
"""
再说说回调这个玩意儿
python的函数不需要指针就能调用，那么还是要写的有条理不然会乱套
在一个类中的函数要作为给外部调用的时候我们称之为回调函数
因为没有类这个概念了，所以回调函数传递的就是名称，那么在实例化之后需要传递的东西就有点难理解
其实我们实例化一个类之后，我们只要在这个类里调用另一个类的注册函数就好，或者在这个类初始化的时候就传递回调函数
那么同样的一个类里面有多个回调也很正常，比如这个类接收到了一些数据需要分发给好几个其他类，那么有好几个其他类的传递函数，就没啥问题
需要注意的是这个回调还是得按照顺序来，也就是程序逻辑上不能乱。
"""
...
class PARAMS:
    ...
    # 雷达融合T265频率除数 python管理雷达:400/PACKLEN/RADAR_SKIP/RS_SKIP/FUSION_SKIP ROS不使用该参数
    FUSION_SKIP = 2
...
class Navigation(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "mapper" in kwargs:
            ...
            self.mapper = None
        ############### PID #################
        self.navi_speed = 30  # 导航速度 / cm/s
        self.pid_tunings = {  # PID参数 (仅导航XY使用)
            "default": (0.6, 0.001, 0.08),  # 默认
            "navi": (1.5, 0.0, 0.02),  # 导航
            "hover": (0.7, 0.0, 0.03),  # 悬停
            "land": (0.85, 0.0, 0.02),  # 降落
        }
        self.height_pid = PID(
            0.8, 0.0005, 0.01, setpoint=0, output_limits=(-30, 30), auto_mode=False
        )
        self.navi_x_pid = PID(
        ...
        self.yaw_pid = PID(
            0.7, 0.0, 0.08, setpoint=0, output_limits=(-30, 30), auto_mode=False
        )
        #####################################
        ...
        # 基地点(雷达坐标系)(Note:仅用于雷达扫网定位,建图则不需要) / cm
        self.basepoint: Any = np.array([0.0, 0.0])
        #####################################
    ...
    def calibrate_basepoint(self, wait=True) -> np.ndarray:
        ...
        重置基地点到当前雷达位置 / cm
        """
        if wait and not self.radar.rt_pose_update_event.wait(0.4):
    ...
    def start(self, mode="fusion"):
        ...
        self._thread_list.append(
            threading.Thread(target=self._keep_height_task, daemon=True)
        )
        self._thread_list[-1].start()
        self._thread_list.append(
            threading.Thread(target=self._navigation_task, daemon=True)
        )
        self._thread_list[-1].start()
    ...
    def switch_navigation_mode(
        self, mode: Literal["radar", "rs", "fusion", "fusion-ros"]
    ):
        """
        切换导航模式
        radar: 仅雷达扫网定位
        rs: 仅T265定位
        fusion: 雷达扫网定位辅助T265定位(有趣)
        fusion-ros: ROS建图辅助T265定位
        """
        assert mode in ("radar", "rs", "fusion", "fusion-ros"), (
        ...
        while self.running:
            try:
                ...
                if not (
                    ...
                    """ print("keep_height_flag")
                    print(self.keep_height_flag)
                    print("mode.value")
                    print(self.fc.state.mode.value)
                    print("unlock.value")
                    print(self.fc.state.unlock.value) """
                    if not paused:
                        paused = True
    ...
    def _get_t265_pose(self, wait=True) -> Optional[Tuple[float, float, float, bool]]:
        ...
        if self._t265_trans_args is None:
            ...
            position, eular = self.rs.get_pose_in_secondary_frame(
                self._t265_trans_args, as_eular=True
            )
        ...
        logger_dbg.debug(
            f"[NAVI] RealSense pose: {current_x}, {current_y}, {current_yaw}, {available}"
        )
        ...
        while self.running:
            try:
                if self._navigation_mode == "radar":
                    ...
                    raise ValueError(
                        f"Unknown navigation mode: {self._navigation_mode}"
                    )
                if pose is None:
                ...
                logger_dbg.debug(
                    f"[NAVI] Pose: {self.current_x}, {self.current_y}, {self.current_yaw}"
                )
                if not (
                ...
                logger_dbg.info(
                    f"[NAVI] Pose PID output: {out_x}, {out_y}, {out_yaw},input:{self.current_x}, {self.current_y}, {self.current_yaw}"
                )
            except Exception as e:
    ...
    def calibrate_realsense(self, wait=True):
        ...
        logger_dbg.info(
            f"[NAVI] Calibrate T265: radar={self.radar.rt_pose} dz={dx}, dx={dy}, dy={dz}, dyaw={dyaw}"
        )
    ...
    def calibrate_realsense_ros(self, wait=True):
        ...
        # type: ignore
        logger_dbg.info(
            f"[NAVI] Calibrate T265: map={self.mapper.position} dz={dx}, dx={dy}, dy={dz}, dyaw={dyaw}"
        )
        self._t265_trans_args = self.rs.establish_secondary_origin(
            force_level=True, z_offset=dx, x_offset=dy, yaw_offset=dyaw, y_offset=dz
        )
    ...
    def direct_set_waypoint(self, waypoint):
        ...
        """
        self.navi_x_pid.tunings = self.pid_tunings["default"]
        self.navi_y_pid.tunings = self.pid_tunings["default"]
    ...
    #     for n, point in enumerate(traj_list):
    #         self.stopn=n
    #         while not self._reached_waypoint(15):
    #             time.sleep(dt / 2 if dt is not None else 0.02)
    #             if not self.traj_running_event.is_set():
    #                 self.traj_running_event.set()
    #                 logger.debug("[NAVI] Trajectory task forced to stop")
    #                 self.traj_list_before_stop = traj_list[n:]
    #                 return
    #             if not (self.running and self.navigation_flag):
    #                 logger.debug("[NAVI] Trajectory task forced to stop1")
    #                 self.traj_list_before_stop = traj_list[n:]
    #                 return
    #         if dt is not None:
    #             while time.perf_counter() - start_t < dt * (n + 1):
    #                 time.sleep(dt / 2)
    #         x, y = float(point[0]), float(point[1])
    ...
    #     self.navi_y_pid.output_limits = (-self.navi_speed, self.navi_speed)
    #     if wait:
    #         self._trajectory_task(traj_list,dt)
    #         self.wait_for_waypoint()
    #     else:
    #         t = threading.Thread(
    #             target=self._trajectory_task, args=(traj_list, dt), daemon=True
    #         )
    #         t.start()
    #         self._thread_list.append(t)
    #         self.traj_running_event.wait()
    ...
    def navigation_to_waypoint_relative(self, waypoint_rel, *args, **kwargs):
        ...
        self.navigation_to_waypoint(
            self.current_point + np.asarray(waypoint_rel), *args, **kwargs
        )
    ...
    def pointing_takeoff(self, point, target_height=140):
        ...
        time.sleep(2)  # 等待电机启动
        self.fc.take_off(80)
        self.fc.wait_for_takeoff_done(timeout_s=3)
    ...
    def pointing_landing(self, point):
        ...
        time.sleep(0.5)
        self.set_height(60)
        self.wait_for_height(timeout=3)
        self.set_height(30)
        time.sleep(1.5)
        self.set_height(20)
        time.sleep(2)
    ...
    def wait_for_waypoint(self, time_thres=2, pos_thres=5, timeout=15):
        """
        等待到达目标点
