...
class ICPM(object):
    ...
    def match(self, points, debug=False, debug_save_img=False, debug_size=500):
        ...
        while dError > self._EPS:
            count += 1
            indexes, error = ICPM._nearest_neighbor_association(
                self._template_points, points
            )
            Rt, Tt = ICPM._svd_motion_estimation(
                self._template_points[:, indexes], points
            )
    ...
    def _plot_points(self, previous_points, current_points, save_img, size):
        ...
        select = np.all(
            (previous_points >= -size // 2) & (previous_points < size // 2), axis=0
        )
        previous_points = previous_points[:, select] + size // 2
        select = np.all(
            (current_points >= -size // 2) & (current_points < size // 2), axis=0
        )
