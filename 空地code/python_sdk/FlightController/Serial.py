        ...
        while self._ser.in_waiting > 0:
            if not self._reading_flag:
                ...
                if self._in_waiting_buffer[-len(self._read_start_bit) :] == self._read_start_bit:
                    ...
                    self._pack_length = -1  # 读到帧头后重置包长度
                    self._in_waiting_buffer = bytes()
            else:
                if self._pack_length == -1:
                    self._pack_length = self._ser.read(1)[0]
                if self._ser.in_waiting >= self._pack_length:   # 判断是否接收到完整包长度以上的数据
                    data = self._ser.read(self._pack_length)    # 读取完整包长度的数据
                    self._reading_flag = False
                    checksum = (sum(data) + self._pack_length + self._read_start_bit_sum) & 0xFF    # 计算校验和
                    received_checksum = self._ser.read(1)[0]    # 读取校验和
