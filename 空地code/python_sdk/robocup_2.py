...
MISSION_POINTS: np.ndarray = np.array(
    [ [80,-180],[100,-580], [-190, -580], [-230, -210]])

NEXT_MISSION_PATH:np.ndarray=np.array([[ 280,-650   ],[  280,-820  ],[100,-820]])
NEXT_MISSION_POINT: np.ndarray = np.array([ 250,-770 ])
SPEED = 18
HEIGHT = 85
...
class Mission(object):
    def __init__(self, *args, **kwargs):
        ...
        self.package_pos = {0: [7, 12], 1: [7, -24], 2: [10, 0]}  # 每个包需要的偏置，还没写还没测
        self.ob_count = 0  # 确认有
        ...
        self.obstacle_gen = ObstacleGenerator(
            x_range1=-450, x_range2=450, y_range1=-900, y_range2=200)
    ...
    def init_obs(self,N=0):
       ...
       #         45, 135, 3)[0].to_xy() / 10)
        ...
        for retry in range(trymax):
            for i in range(10):
                img = self.radar.map.output_cloud(0.1, 1600)
    ...
    def find_door(self):
        ...
        while attempt_count < MAX_ATTEMPTS:
            ...
            points = self.radar.map.find_nearest_with_stable_ext_point(
                125, 225, 2, 2000)
            ...
            if len(points) < 2:
                ...
                attempt_count += 1
                continue  
            point1 = points[0].to_xy() / 10
            point2 = points[1].to_xy() / 10

            distance = np.sqrt((point2[0] - point1[0])**2 + (point2[1] - point1[1])**2)
            if 65 <= distance <= 95:
                print(f"找到一扇门，距离为 {distance:.2f}。正在计算中点。")
                midpoint_x = (point1[0] + point2[0]) / 2
                midpoint_y = (point1[1] + point2[1]) / 2
                midpoint = (midpoint_x+navi.current_x, midpoint_y+navi.current_y)
                logger.info(f"中点坐标为：{midpoint}")
                print(f"找到点1:{point1},点2：{point2}")
                return midpoint  # 找到后立即返回中点

            else:
                print(f"距离 {distance:.2f} 不在 70-90 的范围内。")
                attempt_count += 1
    ...
    def next_mission(self):
        ...
        for step in range(2):
            ...
            point=self.find_door()
            if point is not None:
                logger.info(f"找到了门中点{point}")
                navi.navigation_to_waypoint((navi.current_x,point[1]))
        ...
        if step==1:
            logger.info("穿越完成")
            navi.navigation_to_waypoint(navi.current_x,NEXT_MISSION_POINT[1])
            navi.pointing_landing(navi.current_point)
