...
NEXT_MISSION_PATH:np.ndarray=np.array([[ 400,-650   ],[  400,-810  ],[250,-810]])
...
class Mission(object):
    def __init__(self, *args, **kwargs):
        ...
        self.obstacle_gen = ObstacleGenerator(
            x_range1=-500, x_range2=500, y_range1=-1000, y_range2=200)
    ...
    def add_obs(self, x1, y1):
        self.obstacle_gen.add_obstacle(
            'circle', cx=x1, cy=y1, radius=70, filled=True)
    ...
    def next_mission(self):
        ...
        if step==1:
            logger.info("穿越完成")
            navi.navigation_to_waypoint((int(navi.current_x-15),NEXT_MISSION_POINT[1]))
            navi.pointing_landing(navi.current_point)
    def run(self):
        fc = self.fc
        cam = self.cam
