...
from trash.onnx_test import infer_onnx
...
class Mission(object):
    def __init__(self, *args, **kwargs):
        ...
        self.gpio_com = [170, 1, 90, 0,0, 255]#激光/舵机/灯/蜂鸣器 发1动 0灭
        self.terminal_rxbuffer = [0]
        self.session = ort.InferenceSession("./model/best.onnx")
        self.mud_point = None
        self.fire_point = []
        self.lake_point = None
        self.water=False
        if self.use_serial:
            self.serial_terminal = Serial_car(device="cp2102", baudrate=115200, rx_length=3)
    ...
    def gpio_set_laser(self,flag):
        self.gpio_com[1]= flag
    def gpio_set_servo(self,flag):
        if flag == 1:
            self.gpio_com[2]=0
        else:
            self.gpio_com[2]=90
    def gpio_set_LED(self,flag):
        self.gpio_com[3]=flag
    def gpio_set_buzzer(self,flag):
        self.gpio_com[4]=flag
    def detect(self, img):
        img_center_x = int(img.shape[1]/2)
        img_center_y = int(img.shape[0]/2)
        # boxes, labels = infer_and_draw(img, self.session, 0.5)

        res = infer_onnx(model_session=self.session, inp_image=img,
                         inpHeight=320, confThreshold=0.35, nmsThreshold=0.2)
        res_len = len(res)
        if res_len == 0:
            return None
        else:
            for item in res:
                ...
                print(label)
                if abs(center_x - img_center_x) < 160 and abs(center_y - img_center_y) < 160:
    ...
    def run(self):
        ...
        for point in mission_points_list:
            ...
            navi.wait_for_waypoint(time_thres=1.5, pos_thres=7, timeout=10)
            self.gpio_set_laser(0)
            ...
            if point_res == 2:
                ...
                logger.info(f"find lake point {point}")
                if len(self.fire_point) is not 0:
                    self.fire_handler()  # 永远只会在后出现的点触发
            if point_res == 3:
                self.mud_point = point
                logger.info(f"find mud point {point}")
                self.mud_handler()
            if point_res == 4:
                self.gpio_set_buzzer(1)
                self.gpio_set_LED(1)
                ...
                time.sleep(0.3)
                self.fire_point.append(point)
                logger.info(f"find fire point {point}")
                self.gpio_set_buzzer(0)
