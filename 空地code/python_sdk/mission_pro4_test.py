...
class Mission(object):
    ...
    def init_obs(self):
       ...
       #         45, 135, 3)[0].to_xy() / 10)
        ...
        #     stable_points = self.radar.map.find_nearest_with_stable_ext_point(from_=40, to_=110, num=3, range_limit=4500,angle_tolerance=3,dist_tolerance=500)
        #     if len(stable_points) >= 3:
        #         for i in range(3):
        #             point.append(stable_points[i].to_xy() / 10)
        #         break
        #     else:
        #         logger.warning(f"尝试 {retry+1}/{max_retry}: 只找到 {len(stable_points)} 个点")
        #         time.sleep(0.5)
        # else:
        #     logger.error("无法获取足够的稳定点")
        # for i in range(3):
        #     point.append(self.radar.map.find_nearest_with_stable_ext_point(
        #         from_=40, to_=110, num=3, range_limit=4500,angle_tolerance=3,dist_tolerance=500)[i].to_xy() / 10)
        for i in range(5):
          point=self.radar.map.find_nearest_and_cluster(from_=40,to_=130,step=3,dis=4400)
          time.sleep(0.2)
        point = np.array(point)/10
        logger.debug(f"loc: {point}")
        degree = []
        for i in range(len(point)):
            degree.append(int(np.arctan2(point[i][0], point[i][1])))

            self.add_obs(int(point[i][0] + r*np.sin(degree[i])),
                         int(point[i][1] - r*np.cos(degree[i])))
            logger.info(f"{int(point[i][0] + r*np.sin(degree[i])),int(point[i][1] - r*np.cos(degree[i]))}")
        self.obs()
    ...
    def run(self):
       ...
       # set_cam_autowb(cam, True)
        ...
        next_points = MISSION_POINTS[1]
        for i, points in enumerate(MISSION_POINTS):
            logger.info(f"[MISSION] Go to {i+1} point {points}")
            if i+1 < len(MISSION_POINTS):
                next_points = MISSION_POINTS[i+1]
                path = self.Astar_plan(points, next_points)
                if path is not None and len(path):
                    path = self.path_3d(path)
                else:
                    logger.warning(
                        f"[MISSION] Path planning failed for point {i+1}. Skipping navigation.")
                    break
                navi.navigation_follow_trajectory(path, wait=True, dt=0.15)
