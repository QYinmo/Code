...
MISSION_POINTS: np.ndarray = np.array(
    [ [-100, -120], [100, -270],[320,-170],[250,-470]])  # 顺时针1234
...
######## 视觉与追踪######
    ...
    def target(self):
        ...
        if self.found_hospital is False:
            if self.area_res in {0,4}:
                logger.info("没找到红十字，帐篷或者桥,直接下一个点")
                return
        else:
            if self.area_res in {0, 1,4}:
                logger.info("帐篷,地堡或者桥,直接下一个点")
                return
        while True:
    ...
    def run(self):
       ...
       # set_cam_autowb(cam1, True)
        ...
        if not self.found_hospital:
            ...
            self.hospital_pos=HOSPITAL_PRE_POINT
            self.found_hospital=True
