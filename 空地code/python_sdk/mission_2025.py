...
import math # 新增：导入math库以使用ceil函数
...
# 新增：最大插值步长，单位为厘米和米
MAX_STEP_SIZE_CM = 5
MAX_STEP_SIZE_M = MAX_STEP_SIZE_CM / 100.0
...
# ------处理与任务-------
    ...
    def record(self, positon, statistic):  # 记录数据
        navi = self.navi
        start_point = P((navi.current_x+25)//50, (navi.current_y+25)//50)
...
# -------run------
    ...
    def get_route(self):  # 根据串口信号转化为任务轨迹
            ...
            if list_of_processed_arrays:
                ...
                # 创建新的路径列表
                new_path_waypoints = []
                new_path_waypoints.append(np.array([0, 0, HEIGHT]))
                for point in turn_points:
                    new_path_waypoints.append(np.array(point))
                
                new_path_waypoints.append(np.array([0, 0, HEIGHT]))
                logger.info(f"关键路径点{new_path_waypoints}")
    ...
    def run(self):
        ...
        for i in range(len(mission_path)-1):
            last_p = mission_path[i]
            next_p = mission_path[i+1]
            length = np.linalg.norm(next_p - last_p)
            
            # 计算插值步数，确保每一步的距离最大为5厘米
            num_steps = math.ceil(length / MAX_STEP_SIZE_CM)
            if num_steps <= 1:
                # 如果距离太短，直接使用两个点
                way_path = [last_p, next_p]
                dt_for_segment = length / SPEED
            else:
                way_path = np.linspace(last_p, next_p, num_steps)

                dt_for_segment = (length / SPEED) / (num_steps - 1)

            logger.info(f"正在从点 {i} 到点 {i+1} 飞行, 距离: {length:.2f}cm, 插值步数: {len(way_path)}, 每步时间: {dt_for_segment:.4f}s")
            
            # 导航到插值后的路径
            navi.navigation_follow_trajectory(np.array(way_path), wait=False, dt=0.15)
            self.list=way_path[navi.stopn:]
            while True:
                if  not navi.traj_running_event.is_set() and not self.stop_status:
                    break
                current_points=[navi.current_x,navi.current_y]
                self.points = (max(0, int((current_points[0]+25)//50)), max(0, int((current_points[1]+25)//50))
)
                ...
                if self.points==(0,0)and time_num>=1000:
                    logger.info("回到起点，准备降落")
                    break

                time_num+=1
                time.sleep(0.15)
            time.sleep(0.08)
        self.land_45(land_point=self.saved_point)
