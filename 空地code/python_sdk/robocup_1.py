...
MISSION_POINTS: np.ndarray = np.array(
    [ [250,-150], [-100, -280], [-150, -480],[280,-550]])#顺时针1234

NEXT_MISSION_PATH:np.ndarray=np.array([[ 270,-650   ],[  270,-820  ],[100,-820]])
NEXT_MISSION_POINT: np.ndarray = np.array([100,-810])#-330，-855
SPEED = 20
HEIGHT = 85
...
class Mission(object):
    def __init__(self, *args, **kwargs):
        ...
        self.obstacle_gen = ObstacleGenerator(
            x_range1=-400, x_range2=400, y_range1=-900, y_range2=200)
    ...
    def init_obs(self,N=0):
        ...
        self.add_obs(-25,-405)
        self.obstacle_gen.add_obstacle('rectangle',x1=220,y1=-320,x2=400,y2=-400,filled=True)
        point = []
        trymax=5
        # r = 5
    ...
    def find_door(self):
        ...
        while attempt_count < MAX_ATTEMPTS:
            ...
            if len(detected_walls) == 0:
                ...
                print("\n中间两个点的坐标是：")
                print(f"  - 第一个中间点: ({middle_point1[0]:.2f}, {middle_point1[1]:.2f})")
                print(f"  - 第二个中间点: ({middle_point2[0]:.2f}, {middle_point2[1]:.2f})")
            ...
            if radar_point is not None:
                ...
                if num==1:
                    if point[1]>=NEXT_MISSION_POINT[1]:
                        navi.navigation_to_waypoint((navi.current_x,point[1]-72))
                        navi.wait_for_waypoint(time_thres=0.8,pos_thres=8)
                        navi.navigation_to_waypoint((point[0]-dis,point[1]-72))
                        navi.wait_for_waypoint(time_thres=0.8,pos_thres=8)

                    else    :
                        navi.navigation_to_waypoint((int(navi.current_x),point[1]+72))
                        navi.wait_for_waypoint(time_thres=0.8,pos_thres=8)
                        navi.navigation_to_waypoint((point[0]-dis,point[1]+72))
                        navi.wait_for_waypoint(time_thres=0.8,pos_thres=8)   
                else:
