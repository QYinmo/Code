...
class Mission(object):
    ...
    def identify_area(self, img):
        result = vision_client_pro4.get_detection_results(
            block=True, timeout=0.2)
        if result.get('success'):
            has_detection = result.get('has_detection', False)
            if has_detection:
                logger.info("检测到目标：")
                for res in result.get('final_results', []):
                    print(
                        f"类别: {int(res[2])}, 坐标: ({res[0]:.5f}, {res[1]:.5f}),pid坐标:{(res[0]-0.5)*img.shape[1], (res[1]-0.5)*img.shape[0]} 置信度: {res[3]:.5f}")
