...
try:
    ...
    P_left = np.array([[stereo_focal_px, 0, stereo_cx, 0], [0, stereo_focal_px, stereo_cy, 0], [0, 0, 1, 0]])
    P_right = P_left.copy()
    ...
    Q = np.array(
        [[1, 0, 0, -(stereo_cx - max_disp)], [0, 1, 0, -stereo_cy], [0, 0, 0, stereo_focal_px], [0, 0, -1 / T[0], 0]]
    )
    ...
    while True:
        ...
        if valid:
            ...
            disparity = (
                stereo.compute(center_undistorted["left"], center_undistorted["right"]).astype(np.float32) / 16.0
            )
