...
from FlightController.Solutions.PathPlanner2 import PFBPP, TrajectoryGenerator, AStar, ObstacleGenerator
...
MISSION_POINTS: np.ndarray = np.array(
    [[-100, -120], [100, -270], [320, -170], [250, -470]])

NEXT_MISSION_PATH: np.ndarray = np.array(
    [[370, -650], [370, -800], [200, -800]])
...
def sample_path(path_array: np.ndarray, sample_interval: int = 5) -> np.ndarray:
    """
    对路径数组进行采样，每 N 个点取一个点，并保证起点和终点存在。

    Args:
        path_array (np.ndarray): 原始路径，形状为 (N, 2)。
        sample_interval (int): 采样间隔 N。

    Returns:
        np.ndarray: 采样后的路径数组。
    """
    if not isinstance(path_array, np.ndarray) or path_array.ndim != 2 or path_array.shape[1] != 2:
        logger.error("Input path_array must be a NumPy array of shape (N, 2).")
        return np.array([])

    path_length = len(path_array)

    if path_length <= 2:
        logger.warning("Path length is 2 or less. Returning original path.")
        return path_array
    ...
    # 2. 均匀采样中间点
    # 从索引 1 开始，每隔 sample_interval - 1 步取一个点
    # 这样可以确保如果 sample_interval=5，我们取的是 0, 5, 10, ...
    # 也就是 i * sample_interval 处的点
    ...
    if end_index not in sampled_indices:
        sampled_indices.append(end_index)

    # 4. 去除重复的索引并按顺序排列 (虽然在上面的逻辑中不应该有重复，但为了健壮性)
    sampled_indices = sorted(list(set(sampled_indices)))

    # 5. 提取采样的点
    sampled_path = path_array[sampled_indices]

    logger.info(f"Original Path Length: {path_length}")
    logger.info(f"Sampled Path Length: {len(sampled_path)}")
...
######## 视觉与追踪######
                ...
                if f:
                    ...
                    if state == 0 and abs(dx) < 15 and abs(dy) < 15:  # 到位
                        if self.area_res == data:
                            ...
                            if self.area_res == 3:
                                logger.info("这是车")
                            to_point = (point[0] + self.package_pos[self.package_num]
                                        [0], point[1] + self.package_pos[self.package_num][1])
                            self.navi.navigation_to_waypoint(
                                to_point, wait=True)
                            state = 1
                            continue
                        else:
            ...
            if state == 0:
                ...
                if f:
                    if state == 0 and abs(dx) < 10 and abs(dy) < 10:  # 到位
                        logger.info(f"[MISSION] Reached point at {point}")
                        to_point = (point[0] + self.package_pos[0]
                                    [0], point[1] + self.package_pos[0][1])
    ...
    def find_door(self):
        ...
        while attempt_count < MAX_ATTEMPTS:
            ...
            if len(detected_walls) == 0:
                ...
                logger.info("\n中间两个点的坐标是：")
                logger.info(
                    f"  - 第一个中间点: ({middle_point1[0]:.2f}, {middle_point1[1]:.2f})")
                logger.info(
                    f"  - 第二个中间点: ({middle_point2[0]:.2f}, {middle_point2[1]:.2f})")
    ...
    def next_mission(self):
                ...
                if num == 1:
                    if point[1] >= NEXT_MISSION_POINT[1]:
                        navi.navigation_to_waypoint(
                            (navi.current_x, point[1]-70))
                        navi.wait_for_waypoint(time_thres=0.8, pos_thres=8)
                        navi.navigation_to_waypoint(
                            (point[0]-dis, point[1]-70))
                        navi.wait_for_waypoint(time_thres=0.8, pos_thres=8)

                    else:
                        navi.navigation_to_waypoint(
                            (int(navi.current_x), point[1]+70))
                        navi.wait_for_waypoint(time_thres=0.8, pos_thres=8)
                        navi.navigation_to_waypoint(
                            (point[0]-dis, point[1]+70))
                        navi.wait_for_waypoint(time_thres=0.8, pos_thres=8)
        ...
        if step == 0:
            navi.navigation_to_waypoint(
                (int(navi.current_x-200), NEXT_MISSION_POINT[1]))
            navi.wait_for_waypoint(time_thres=0.8, pos_thres=8)
        if step == 1:
            logger.info("穿越完成")
            navi.navigation_to_waypoint(
                (int(navi.current_x-20), NEXT_MISSION_POINT[1]))
            navi.pointing_landing(navi.current_point)
    ...
    def add_hospital_optimized(self):
        ...
        if not self.found_hospital:
            logger.warning(
                "Error: Hospital not found. Skipping coordinate addition.")
            return
        ...
        for i in range(num_points - 1):
            ...
            new_dist = np.linalg.norm(
                P_i - hospital_point) + np.linalg.norm(hospital_point - P_i_plus_1)
            added_distance = new_dist - original_dist
            logger.debug(
                f"Segment {i} -> {i+1}: Original={original_dist:.2f}, New={new_dist:.2f}, Added={added_distance:.2f}")
        ...
        if best_insert_index != -1:
            ...
            if is_hospital_already_near:
                logger.warning(
                    f"Hospital point is too close to an existing mission point. Skipping insertion.")
                return
    ...
    def run(self):
       ...
       # set_cam_autowb(cam1, True)
        set_manual_exporsure(cam1, -6)
        set_manual_exporsure(cam2, -5.5)
        ...
        navi.pointing_takeoff(BASE_POINT, 100)
        navi.wait_for_height()
