...
MISSION_POINTS: np.ndarray = np.array(
    [ [-200, -360], [-560, 140]])  # 顺时针1234

NEXT_MISSION_PATH: np.ndarray = np.array(
    [[-650, -400], [-810, -400], [-810, -250]])
...
class Mission(object):
    def __init__(self, *args, **kwargs):
        ...
        self.obstacle_gen = ObstacleGenerator(
            x_range1=100, x_range2=-1000, y_range1=-500, y_range2=500)
...
####### 障碍与路线规划#########
    ...
    def add_obs(self, x1, y1):
        self.obstacle_gen.add_obstacle(
            'circle', cx=x1, cy=y1, radius=80, filled=True)
...
######## 视觉与追踪######
    ...
    def dxdy_to_fxfy(self):
        ...
        self.found_coordinates.append((fx+x, fy+y))
    ...
    def get_hospital_pos(self):
        if self.found_coordinates:
            ...
            print(f"成功找到的坐标数量: {count}")
            print(f"所有找到的坐标: {self.found_coordinates}")
            print(f"红十字坐标 : {self.hospital_pos}")
        else:
            print("三次寻找操作中，没有找到任何坐标。无法计算平均坐标。")
    ...
    def next_mission(self):
        ...
        if step == 0:
            navi.navigation_to_waypoint(
                (NEXT_MISSION_POINT[0], int(navi.current_y+200)))
            navi.wait_for_waypoint(time_thres=0.8, pos_thres=8)
            logger.info("穿越第一道门,前进2m完成")
        if step == 1:
            logger.info("穿越完成")
            navi.navigation_to_waypoint(
                (NEXT_MISSION_POINT[0], int(navi.current_y+20)))
            navi.pointing_landing(navi.current_point)
    ...
    def run(self):
       ...
       # set_cam_autowb(cam1, True)
        ...
        path_go = self.Astar_plan(
            [int(self.navi.current_x//5*5), int(self.navi.current_y//5*5)], NEXT_MISSION_PATH[0])
        while True:
            logger.info(f"当前坐标：x:{navi.current_x},y:{navi.current_y}")
            time.sleep(0.5)
        ...
        # time.sleep(0.3)
        # self.find_hospital = False
        # navi.navigation_to_waypoint((0, 150), wait=True)
