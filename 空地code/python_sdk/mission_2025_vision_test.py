...
from vision.Vision_plus import vision_debug, get_area_in_frame, get_ROI, debug_imshow, find_red_area, HighPrecisionFPS
...
def id_function(img, model):
    try:
        ...
        results = model.predict(source=img, show=False, conf=conf_thres, save=False,
                                verbose=False, stream=True, device='cpu', iou=0.3)
...
while True:
    ...
    logger.debug(
        f"f:{f}all_detected_objects:{all_detected_objects}class_counts:{class_counts}")
    key = cv2.waitKey(1)
    if key == ord('q'):
        break
