...
MODEL_PATH = r"/home/n1/workplace/Drone_maindev/python_sdk/final.pt"
CONF_THRES = 0.7
...
def detect_objects_in_frame(img, model):
    ...
    try:
                ...
                if result.boxes is not None and len(result.boxes.cpu().numpy()) > 0:
                    ...
                    for i in range(len(classes)):
                        all_detected_objects.append(
                            {
                                "dx": float(float(xy[i][0])-img.shape[1]/2),
                                "dy": float(float(xy[i][1])-img.shape[0]/2),
                                "class_id": int(classes[i]),
                                "class_name": ANIMAL_CLASS.get(
                                    int(classes[i]), "Unknown"
                                ),
        ...
        # Convert class_counts keys to class names for better readability
        class_counts_named = {}
        for cls_id, count in class_counts.items():
            class_name = ANIMAL_CLASS.get(cls_id, f"Unknown_{cls_id}")
            class_counts_named[class_name] = count

        return True, all_detected_objects, class_counts_named, annotated_frame
...
def aggregate_results(results_queue):
    """
    对最新N帧的检测结果进行聚合，返回最稳定的结果。
    ...
    返回格式:
    {
        ...
        "class_name": str,
        "frequency": int,  # 出现次数
        "total_count": int,  # 总检测目标数
    }
    如果队列为空或无检测结果，则返回 None。
    """
    ...
    # 统计每种目标类别在所有帧中的总目标数
    class_total_counts = Counter()

    for result_frame in results_queue:
        if result_frame["has_detection"]:
            for obj in result_frame["detected_objects"]:
                class_id = obj["class_id"]
                class_total_counts[class_id] += 1

            # 统计这一帧中检测到的所有类别
            frame_classes = {obj["class_id"]
                             for obj in result_frame["detected_objects"]}
    ...
    # 找到出现次数最多的类别
    max_frequency = 0
    for cls_id, freq in class_frame_counts.items():
        if freq > max_frequency:
            max_frequency = freq

    # 找出所有与最高频率相同的类别
    top_classes = [cls_id for cls_id,
                   freq in class_frame_counts.items() if freq == max_frequency]

    # 如果只有一个最高频率的类别，直接返回
    if len(top_classes) == 1:
        cls_id = top_classes[0]
        return {
            "class_id": cls_id,
            "class_name": ANIMAL_CLASS.get(cls_id, "Unknown"),
            "frequency": max_frequency,
            "total_count": class_total_counts.get(cls_id, 0),
        }

    # 如果有多个最高频率的类别，根据总目标数进行二次筛选
    if len(top_classes) > 1:
        top_by_count = []
        max_total_count = 0
        ...
        for cls_id in top_classes:
            if class_total_counts.get(cls_id, 0) == max_total_count:
                top_by_count.append(cls_id)

        # 如果依然有多个并列，随机选择一个
        final_cls_id = random.choice(top_by_count)
        return {
            "class_id": final_cls_id,
            "class_name": ANIMAL_CLASS.get(final_cls_id, "Unknown"),
            "frequency": max_frequency,
            "total_count": class_total_counts.get(final_cls_id, 0),
        }
...
def detection_loop():
    ...
    while detection_running:
        try:
            ...
            # Detect objects
            has_detection, detected_objects, class_counts, annotated_frame = (
            ...
            current_frame_results = {
                ...
                "has_detection": has_detection and len(detected_objects) > 0,
                "detected_objects": detected_objects,
                "class_counts": class_counts,
                "fps": current_fps,
            }
            results_queue.append(current_frame_results)

            # 对队列中的结果进行聚合
            aggregated_result = aggregate_results(results_queue)

            # Update latest results
            latest_results.update(
                {
                    "timestamp": current_frame_results["timestamp"],
                    "has_detection": current_frame_results["has_detection"],
                    "detected_objects": detected_objects,  # 保持原始检测结果用于详情展示
                    "class_counts": class_counts,
                    "fps": current_fps,
                    "aggregated_result": aggregated_result,  # 更新聚合结果
                }
            )
            ...
            if latest_results["has_detection"]:
                for obj in detected_objects:
                    logger.info(
                        f"Detected: {obj['class_name']}, Position: ({obj['dx']:.2f}, {obj['dy']:.2f}), Confidence: {obj['confidence']:.3f}"
                    )
