...
def find_red_area(img) -> Tuple[bool, float, float]:
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    RED_UPPER = np.array([10, 255, 255])
    RED_LOWER = np.array([0, 43, 46])
    RED_UPPER2 = np.array([180, 255, 255])
    ...
    filtered_contours = [cnt for _, cnt in sorted_contours if _ >= MIN_AERA]
    if _DEBUG:
...
#             families=families, quad_sigma=quad_sigma, nthreads=nthreads, decode_sharpening=sharpening
#         )
