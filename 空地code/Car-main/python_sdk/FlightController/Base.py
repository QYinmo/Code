...
class FC_State_Struct:
    ...
    motor_x_speed = Byte_Var("s16", int, value_multiplier=0.0001)
    motor_y_speed = Byte_Var("s16", int)
    motor_z_speed = Byte_Var("s16", int, value_multiplier=0.0001)
...
class FC_Base_Uart_Comunication(object):
        ...
        if not block_until_connected:
            ...
            while True:
                try:
                    ...
                    time.sleep(1)
        self._reader = SerialReaderBuffered(self._ser, [0xAA], [0XFF])
    ...
    def send_data_to_fc(
        ...
        send_data = b"\xaa" + send_option.to_bytes(1, "little") + data     # 包含帧头
        send_data += (sum(send_data) & 0xFF).to_bytes(1, "little")  # add checksum
        send_data += b"\xff"    # 包含帧尾
        if not no_lock:
        ...
        while self.running:
            try:
                while self._reader.read():
                    # logger.info(self._reader.data)
                    self._update_fc_data(self._reader.data)
                    time.sleep(0.01)  # 降低CPU占用

            except SerialException: # 错误处理
    ...
    def _update_fc_data(self, _data: bytes):
        ...
        if cmd == 0x01:  # 状态回传
            ...
            if callable(self._wireless_callback):
