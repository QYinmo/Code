...
def plot_car(x, y, yaw, steer=0.0, cabcolor="-r", truckcolor="-k"):  # pragma: no cover
    outline = np.array(
        [
            [-BACKTOWHEEL, (LENGTH - BACKTOWHEEL), (LENGTH -
                                                    BACKTOWHEEL), -BACKTOWHEEL, -BACKTOWHEEL],
...
def stanley_control(state, cx, cy, cyaw, last_target_idx):
    ...
    theta_d = np.arctan2(k * error_front_axle, 200)
    # Steering control
    logger.debug(f"theta_e{theta_e}theta_d{theta_d}")
    delta = 1.3*theta_e + 1.4*theta_d
...
def calc_target_index(state, cx, cy):
    ...
    logger.debug(f"error_front_axle{error_front_axle}")
...
def main():
    ...
    while max_simulation_time >= time and last_idx > target_idx:
        ...
        t.append(time)
        print(
            f"state.x: {state.x:.2f}, state.y: {state.y:.2f}, state.yaw: {state.yaw:.2f}, state.v: {state.v:.2f}")
...
class Controller(object):
    def __init__(self, cx, cy, cyaw, ck, target_speed, dl=0.01, initial_state=None) -> None:
        ...
        logger.debug(
            f"len(cx): {len(cx)}, len(cy): {len(cy)}, len(cyaw): {len(cyaw)}, len(ck): {len(ck)}")
        self.sp = calc_speed_profile(cx, cy, cyaw, target_speed)
        self.state = State(x=cx[0], y=cy[0], yaw=cyaw[0],
                           v=target_speed) if initial_state is None else initial_state
        self.goal = [cx[-1], cy[-1]]
    ...
    def iter_output(self) -> Generator[Tuple[float, float], None, None]:
        ...
        while True:
            ...
            di, self.target_idx = stanley_control(
            ...
            if show_animation:  # pragma: no cover
                ...
                plt.plot(self.cx[self.target_idx],
                         self.cy[self.target_idx], "xg", label="target")
